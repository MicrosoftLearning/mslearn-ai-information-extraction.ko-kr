...
1. Azure Portal에서 Azure AI 검색 리소스를 찾습니다. 그런 다음, **개요** 페이지에서 **데이터 가져오기**를 선택합니다.
    ...
    ![데이터 가져오기가 강조 표시된 Azure Search Service의 스크린샷.](./media/overview-panel.png)
    
    > **참고:** Azure AI Search 리소스의 **개요** 페이지에서 도구 모음은 다음 두 가지 옵션을 제공합니다.  
    > - **데이터 가져오기**(클래식 환경)  
    > - **데이터 가져오기(신규)**(새 환경)  
    >  
    > 또한 개요 섹션 아래의 **시작** 패널에는 **새 UI**로 리디렉션하는 **가져오기** 단추가 포함되어 있습니다.  
    >  
    > 이 과정의 지침은 **클래식 데이터 가져오기 워크플로**를 참조합니다. 혼동을 방지하려면 도구 모음에서 **데이터 가져오기**를 선택해야 합니다.
...
1. 왼쪽 탐색 창의 **검색 관리**에서 새로 만든 **margies-indexer**가 표시되어야 하는 **인덱서** 페이지를 봅니다. 몇 분 정도 기다렸다가 **상태**가 **성공**으로 표시될 때까지 **&#8635;새로 고침**을 클릭합니다.
